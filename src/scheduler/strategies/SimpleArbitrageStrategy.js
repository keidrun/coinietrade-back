...
const commitmentTimeLimitSec =
  process.env.SCHEDULER_COMMITMENT_TIME_LIMIT_SECONDS;
const orderRetryIntervalMSec =
  process.env.SCHEDULER_ORDER_RETRY_INTERVAL_SECONDS * 1000;
...
async function retryPromise(
  ...
  try {
    ...
    if (
      error.code &&
      error.code === ERROR_CODES.API_TEMPORARILY_UNAVAILABLE &&
      retryTimes > 0
    ) {
      logger.warn(`Retrying... after waiting ${retryIntervalMSec} [msec].`);
      logger.debug(
        `Retry interval is ${retryIntervalMSec} [msec] and Retry times is ${retryTimes}.`,
      );
...
class SimpleArbitrageStrategy {
  constructor(argsObj) {
    ...
    this.strategy = argsObj.strategy;
  ...
  async doArbitrage() {
    ...
    try {
      ...
      logger.debug('strategy', this.strategy);
      ...
      if (workingTransactions.length > 0) {
        ...
        logger.warn(
          `Released ${
            releasedTransactions.count
          } transactions and Skipping to make new transactions...`,
        );
      ...
      const transactionMinAmount =
        transactionMinAmountA > transactionMinAmountB
          ? transactionMinAmountA
          : transactionMinAmountB;
      const transactionDigit = String(transactionMinAmount).split('.')[1]
        .length;
      ...
      const possiblePriceLimitA =
        (assetsAmountsA.presentCurrencyAmount - this.assetMinLimit) *
        this.assetRange;
      const possibleCoinAmountLimitA = assetsAmountsA.presentCoinAmount;
      const possiblePriceLimitB =
        (assetsAmountsB.presentCurrencyAmount - this.assetMinLimit) *
        this.assetRange;
      const possibleCoinAmountLimitB = assetsAmountsB.presentCoinAmount;

      if (possiblePriceLimitA < 0 || possiblePriceLimitB < 0) {
        logger.warn(
          `Asset minimum limit exceeded asset ptice and Skipping to make new transactions...`,
        );
        return result.noTransaction();
      }
      ...
      logger.debug(
        `[${this.ExchangeA.getName()}][Coin] ${possibleCoinAmountLimitA}`,
      );
      logger.debug(
        `[${this.ExchangeA.getName()}][Currency] ${possiblePriceLimitA}`,
      );
      logger.debug(
        `[${this.ExchangeB.getName()}][Coin] ${possibleCoinAmountLimitB}`,
      );
      logger.debug(
        `[${this.ExchangeB.getName()}][Currency] ${possiblePriceLimitB}`,
      );
      ...
      const isConditionedToBuyAskBAndSellBidA =
        bestBidPriceA - bestAskPriceB - 2 * bestBidFeeA > 0 ? true : false;
      const isConditionedToBuyAskAAndSellBidB =
        bestBidPriceB - bestAskPriceA - 2 * bestBidFeeB > 0 ? true : false;
      ...
      if (isConditionedToBuyAskBAndSellBidA) {
        ...
        const fixedBuyAmount =
          Math.floor(buyAmount * Math.pow(10, transactionDigit)) /
          Math.pow(10, transactionDigit);
        const fixedSellAmount =
          Math.floor(sellAmount * Math.pow(10, transactionDigit)) /
          Math.pow(10, transactionDigit);
        ...
        target = {
          buy: {
            ...
            laterAssetCoin:
              assetsAmountsB.presentCoinAmount +
              fixedBuyAmount * (1 - transactionFeeRateB),
            laterAssetPrice:
              assetsAmountsB.presentCurrencyAmount - buyPrice * fixedBuyAmount,
          },
          sell: {
            api: this.ExchangeA,
            ...
            laterAssetCoin:
              assetsAmountsA.presentCoinAmount -
              fixedSellAmount * (1 + transactionFeeRateA),
            laterAssetPrice:
              assetsAmountsA.presentCurrencyAmount +
              sellPrice * fixedSellAmount,
          },
        };
        ...
        target = {
          buy: {
            ...
            laterAssetCoin:
              assetsAmountsA.presentCoinAmount +
              fixedBuyAmount * (1 - transactionFeeRateA),
            laterAssetPrice:
              assetsAmountsA.presentCurrencyAmount - buyPrice * fixedBuyAmount,
          },
          sell: {
            api: this.ExchangeB,
            orderPrice: sellPrice,
            ...
            laterAssetCoin:
              assetsAmountsB.presentCoinAmount -
              fixedSellAmount * (1 + transactionFeeRateB),
            laterAssetPrice:
              assetsAmountsB.presentCurrencyAmount +
              sellPrice * fixedSellAmount,
          },
        };
      } else {
        logger.warn(
          `NO match condition and Skipping to make new transactions...`,
        );
        return result.noTransaction();
      }
      ...
      if (target.buy.orderAmount < transactionMinAmount) {
        logger.warn(
          `Too small order amount and Skipping to make new transactions...`,
        );
        return result.noTransaction();
      }
      ...
      const anticipatedProfit = Math.floor(
        target.sell.orderPrice *
          ...
          (1 - target.sell.transactionFeeRate) -
          target.buy.orderPrice *
            target.buy.orderAmount *
            (1 - target.buy.transactionFeeRate),
      );

      logger.info('Order targets =>');
      logger.info(
        `[${target.buy.api.getName()}][BUY][Price] ${target.buy.orderPrice}`,
      );
      logger.info(
        `[${target.buy.api.getName()}][BUY][Amount] ${target.buy.orderAmount}`,
      );
      logger.info(
        `[${target.buy.api.getName()}][BUY][Fee Rate] ${
          target.buy.transactionFeeRate
        }`,
      );
      logger.info(
        `[${target.buy.api.getName()}][BUY][Later Asset Coin] ${
          target.buy.laterAssetCoin
        }`,
      );
      logger.info(
        `[${target.buy.api.getName()}][BUY][Later Asset Price] ${
          target.buy.laterAssetPrice
        }`,
      );
      logger.info(
        `[${target.sell.api.getName()}][SELL][Price] ${target.sell.orderPrice}`,
      );
      logger.info(
        `[${target.sell.api.getName()}][SELL][Amount] ${
          target.sell.orderAmount
        }`,
      );
      logger.info(
        `[${target.sell.api.getName()}][SELL][Fee Rate] ${
          target.sell.transactionFeeRate
        }`,
      );
      logger.info(
        `[${target.sell.api.getName()}][SELL][Later Asset Coin] ${
          target.sell.laterAssetCoin
        }`,
      );
      logger.info(
        `[${target.sell.api.getName()}][SELL][Later Asset Price] ${
          target.sell.laterAssetPrice
        }`,
      );
      ...
      let buyOrderId;
      // => IN_PROGRESS
      const workingBuyTransaction = await transaction.in_progress(
        this.userId,
        buyTransactionId,
      );
      ...
      try {
        ...
        logger.warn(
          `Canceled ${target.buy.api.getName()}'s buy order successfuly because any api problem.`,
        );
      ...
      let sellOrderId;
      // => IN_PROGRESS
      const workingSellTransaction = await transaction.in_progress(
        this.userId,
        sellTransactionId,
      );
      ...
      const isCompletedBuyOrder = await target.buy.api.isCompletedOrder(
        buyOrderId,
      );
      const isCompletedSellOrder = await target.sell.api.isCompletedOrder(
        sellOrderId,
      );
      ...
      if (isCompletedBuyOrder === false && isCompletedSellOrder === true) {
        ...
        const succeededSellTransaction = await transaction.succeeded(
          this.userId,
          sellTransactionId,
        );
        ...
        isCompletedBuyOrder === true &&
        isCompletedSellOrder === false
      ) {
        // TODO: How to recover?
        await target.sell.api.cancelOrder(sellOrderId);
        // => SUCCEEDED but only buy order
        const succeededBuyTransaction = await transaction.succeeded(
          this.userId,
          buyTransactionId,
        );
